...
class NewDeal extends Component {
  ...
  handleChange = input => ({target: {value, checked}}) => {
    ...
    if (input === 'dealTime' || input === 'wantedDealTime') {
      ...
      order.amountTotal = this.state.order.amount + this.state.order.commission + 850 + val
      this.setState({order});

    } else if (input === "commissionPayer") {
      if (value === "buyer") {
        order.commission = 3000
        order.amountTotal = order.amount + order.commission + 850 + order.amountDelivery
...
export default connect(state => ({orders: state.orders}), {addOrder})(withRouter(NewDeal));
